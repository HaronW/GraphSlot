...
class VideoFromTfds:
  ...
  def __call__(self, features: Features) -> Features:
    ...
    if "tracks" in features:
      features_new[self.ragged_boxes_key] = features["tracks"]["bboxes"]
      features_new[self.frames_key] = features["tracks"]["frames"]
      if "segmentations" in features["tracks"]:
        features_new[self.ragged_segmentations_key] = tf.cast(
            features["tracks"]["segmentations"][..., 0], tf.int32)

    try:
      if "video" in features and "segmentations" in features["video"]:
        features[self.segmentations_key] = tf.cast(
            features["video"]["segmentations"][..., 0], tf.int32)
        features_new[self.frames_key] = features["video"]["frames"]
        features["video"] = features["video"]["frames"]
    except:
      pass
    #print(features)
...
class SparseToDenseAnnotation:
  ...
  def __call__(self, features: Features) -> Features:
    ...
    if self.ragged_boxes_key in features:
      # Densify boxes.
