...
from graphslot.datasets.tfds.tfds_dataset_wrapper import MOViData, MOViDataByRank
import graphslot.modules as modules

import graphslot.lib.losses as losses
...
from graphslot.trainers.utils.misc import NativeScalerWithGradNormCount as NativeScaler
...
def get_args():
	...
	# Training config
	adrg('--seed', 42, int)
	adrg('--batch_size', 16, int, help='Batch size per GPU \
	...
	adrg('--output_dir', './output_dir', help="path where to save, empty for no graphslotng.")
...
PYTHONPATH=$PYTHONPATH:./ CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 python3 -m torch.distributed.launch --nproc_per_node=8 graphslot/main.py

PYTHONPATH=$PYTHONPATH:./ CUDA_VISIBLE_DEVICES=0,1,2,3 python3 -m torch.distributed.launch --nproc_per_node=4 graphslot/main.py

PYTHONPATH=$PYTHONPATH:./ CUDA_VISIBLE_DEVICES=0 python3 -m torch.distributed.launch --nproc_per_node=1 graphslot/main.py
